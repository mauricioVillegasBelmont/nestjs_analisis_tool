...
export class PyServicesController {
  ...
  uploadFile(
    ...
    try {
      ...
      files.forEach((file) => {
        const file_name =
          file.filename?.split('.')[0] ?? file.originalname?.split('.')[0];
        ...
        this.pyServicesService.pdf_file_to_html(
          app_dir,
          pdf_file,
          assets_rel_path,
          output_path,
        ...
        success.push(`${file.originalname} uploaded as ${file_name}`);
      });
      return {
        status: 'ok',
  ...
  fromStreamTest(
    ...
    try {
      ...
      files.forEach((file) => {
        ...
        buffer[uniquePrefix]['html'] =
          this.pyServicesService.pdf_buffer_to_html(
            file,
            assets_path,
            uniquePrefix,
          );

        success.push(`${file.originalname} parced to HTML`);
      });
      return {
        status: 'ok',
        statusCode: HttpStatus.OK,
